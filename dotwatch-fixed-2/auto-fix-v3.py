...
# -*- coding: utf-8 -*-
"""
DotWatch Auto-Fix v3
- Fixes dropdown click issue
- Register/Login pages now use main site template (header/nav/footer)
- Run with: python3 auto-fix-v3.py
"""
...
# ===================== FIXED AUTH.JS =====================
AUTH_JS = r"""// ==================== DOTWATCH USER AUTH v3 =======================
(function(){
'use strict';

// Detect prefix from this script's src attribute
...
.user-dropdown-menu{position:absolute;top:calc(100% + 12px);right:0;min-width:270px;background:#fff;border-radius:16px;box-shadow:0 12px 48px rgba(0,0,0,.18);padding:10px 0;opacity:0;visibility:hidden;transform:translateY(-8px);transition:all .25s cubic-bezier(.4,0,.2,1);z-index:99999;border:1px solid rgba(0,0,0,.06);pointer-events:none}
...
const UserAuth={
  ...
  toggleDropdown(e){
    ...
    const d=document.querySelector('.user-dropdown-menu');
    if(!d)return;
  ...
  renderUserAccount(){
    ...
    if(user){
      ...
      const btn=c.querySelector('#ua-toggle');
      const logoutBtn=c.querySelector('#ua-logout');
      if(btn) btn.addEventListener('click',(e)=>this.toggleDropdown(e));
  ...
  init(){
    ...
    document.addEventListener('click',(e)=>{
      if(!e.target.closest('.user-account-wrapper')) this.closeDropdown();
    });
  }
};
...
# ===================== REGISTER PAGE (with main template) =====================
...
<head>
  ...
  <meta name="description" content="ثبت نام در فروشگاه ساعت دات واچ">
...
<!-- Notification Bar -->
<div id="notification-bar" class="notification-bar"><span></span></div>
  ...
  <div class="auth-box">
    ...
    <form id="register-form" novalidate>
      ...
      <div class="form-group">
        ...
        <div class="err" id="err-email">لطفاً ایمیل معتبر وارد کنید</div>
      </div>
...
<script>
  // Populate days & years
  const dSel=document.getElementById('reg-day');
  for(let i=1;i<=31;i++){const o=document.createElement('option');o.value=String(i).padStart(2,'0');o.textContent=i;dSel.appendChild(o);}
  const ySel=document.getElementById('reg-year');
  const cy=new Date().getFullYear();
  for(let i=cy-18;i>=cy-80;i--){const o=document.createElement('option');o.value=i;o.textContent=i;ySel.appendChild(o);}
  ...
  document.getElementById('register-form').addEventListener('submit',function(e){
    ...
    if(!email||!email.includes('@')){document.getElementById('err-email').classList.add('show');document.getElementById('reg-email').classList.add('error');ok=false;}
    if(!day||!month||!year){document.getElementById('err-birthdate').classList.add('show');document.querySelectorAll('.bd-row select').forEach(el=>el.classList.add('error'));ok=false;}
    ...
    UserAuth.login({name:name,email:email,phone:phone,birthdate:year+'-'+month+'-'+day});
...
<main class="auth-section">
  <div class="auth-box">
    ...
    <div class="auth-error" id="login-error">ایمیل یا رمز عبور اشتباه است</div>
...
def main():
    ...
    with open(os.path.join(js_dir, 'auth.js'), 'w', encoding='utf-8') as f:
        f.write(AUTH_JS)
    print("✅ js/auth.js  (fixed dropdown + event listeners)")
    ...
    for filepath in html_files:
        ...
        if 'register/index.html' in rel.replace('\\', '/') or 'login/index.html' in rel.replace('\\', '/'):
            print(f"  ⏭️  Skipped  →  {rel} (handled separately)")
            continue
    ...
    print("""
What's new in v3:
  ✅ auth.js: event listeners attached after render (fixes dropdown click)
  ✅ register/index.html: now uses main site template (header + nav + footer)
  ✅ login/index.html: now uses main site template (header + nav + footer)
...
Next steps:
  ...
  2. Click "ثبت نام" → fill form (birthdate is required!)
  3. After register, click your avatar → dropdown should open!
""")
